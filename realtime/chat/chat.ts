import {UserRealtimeConnection, Message} from '../../Models';
export default (socket:any, io:any)=>{
    socket.on('typing', async(data:any)=>{
        let user = await UserRealtimeConnection.findOne({userId:data.to}, {connectionId:1})
        if(user){
    ...
    socket.on('message', async(data:any)=>{
        let user = await UserRealtimeConnection.findOne({userId:data.to}, {connectionId:1})
        if(user){
            let d = await new Message(data).save()
